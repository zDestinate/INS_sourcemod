...
public OnPluginStart()
{
	...
	PrintToServer("[INS] Hooking round end events for game: %s", theFolder);
...
public OnMapStart()
{
	...
	PrintToServer("[INS] Team Names: %s %s - OnRoundEnd available: %s", teamName1, teamName2, (onRoundEndPossible ? "yes" : "no"));
}
...
public Action:Command_Team(client, args)
{
	if (args < 2)
	{
		if (cstrikeExtAvail)
		{
			ReplyToCommand(client, "[INS] %t", "team usage css");
		}
		else if (TF2)
		{
			ReplyToCommand(client, "[INS] %t", "team usage tf2");
		}
		else
		{
			ReplyToCommand(client, "[INS] %t", "team usage other");
		}
...
public Action:Command_SwitchImmed(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[INS] %t", "swapteam usage");
		return Plugin_Handled;
	}
...
public Action:Command_SwitchDeath(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[INS] %t", "swapteam death usage");
		return Plugin_Handled;
	}
...
public Action:Command_SwitchRend(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[INS] %t", "swapteam rend usage");
		return Plugin_Handled;
	}
	
	if(!onRoundEndPossible)
	{
		ReplyToCommand(client, "[INS] %t", "swapteam rend error");
		return Plugin_Handled;
	}
...
public Action:Command_SwitchSpec(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[INS] %t", "swapteam spec usage");
		return Plugin_Handled;
	}
